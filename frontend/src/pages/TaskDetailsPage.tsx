...
import { useGetTaskQuery, useGetTagsQuery, useUpdateTaskMutation } from '../store/api'
  ...
  return (
    <div className="space-y-6">
      ...
      {task && (() => {
        ...
        const taskTags = task.tags.map((id) => tagsById.get(id)).filter(Boolean) as Tag[]
        ...
        return (
          <div className={[
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="flex shrink-0 flex-wrap gap-2">
                <div className="relative flex items-center">
                  <select
                    value={task.status}
                    onChange={(e) => updateTask({ id: task.id, body: { status: e.target.value as TaskStatus } })}
                    disabled={isUpdating}
                ...
                <span className={`rounded-full px-3 py-1 text-sm font-medium ${PRIORITY_CLASS[task.priority]}`}>
            ...
            {task.description && (
              <p className="mt-4 text-muted-foreground whitespace-pre-wrap">{task.description}</p>
            )}
            ...
            <div className="mt-6 grid gap-3 text-sm">
              ...
              {taskTags.length > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {taskTags.map((tag) => (
                      <span key={tag.id} className="rounded-full bg-secondary px-2.5 py-0.5 text-xs">
                        {tag.name}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                <span className="w-24 shrink-0 font-medium text-foreground">Created</span>
                {new Date(task.createdAt).toLocaleString()}
              </div>

              <div className="flex items-center gap-2 text-muted-foreground">
                <span className="w-24 shrink-0 font-medium text-foreground">Updated</span>
